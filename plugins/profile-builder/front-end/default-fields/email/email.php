...
function wppb_email_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
        ...
        $output = '
		 <input class="text-input default_field_email '. apply_filters( 'wppb_fields_extra_css_class', '', $field ) .' input__field input__field--hoshi"name="email" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="email" id="email" value="'. esc_attr( $input_value ) .'" '. $extra_attr .'  />          <label class="input__label input__label--hoshi input__label--hoshi-color-3" for="email">            <span class="input__label-content input__label-content--hoshi">'.$item_title.$error_mark.'</span>          </label>									';
