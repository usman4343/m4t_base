...
class Profile_Builder_Form_Creator{
	...
	function wppb_output_form_fields( $global_request, $field_check_errors, $form_fields, $called_from = NULL ){
		$output_fields = '';
		if( !empty( $form_fields ) ){			/* echo "<pre>";			print_r($form_fields);			exit; */
			...
			foreach( $form_fields as $field ){
				...
				$output_fields .= apply_filters( 'wppb_output_before_form_field', '<span class="'. $css_class .' input input--hoshi" id="wppb-form-element-'. $field['id'] .'">', $field, $error_var, $this->args['role'] );
				...
				$output_fields .= apply_filters( 'wppb_output_after_form_field', '</span>', $field, $this->args['ID'], $this->args['form_type'], $called_from );
			}
