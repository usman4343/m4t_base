...
function pf_login_form( $args = array() ) {
     ...
     */
    $args = wp_parse_args( $args, add_filter('login_form_defaults', 'wppb_change_login_username_label') );
     ...
     * @param array  $args    Array of login form arguments.
     */
    $login_form_bottom = apply_filters( 'login_form_bottom', 'wppb_login_form_bottom', $args );
	/* echo "<pre>";
	print_r($defaults);
	exit; */
    $form = '
        <form name="' . $defaults['form_id'] . '" id="' . $defaults['form_id'] . '" action="' . esc_url( site_url( 'wp-login.php', 'login_post' ) ) . '" method="post">
            ' . $login_form_top . '
            <span class="input input--hoshi">
                <input type="text" name="log" id="' . esc_attr( $defaults['id_username'] ) . '" class="input__field input__field--hoshi" value="' . esc_attr( $defaults['value_username'] ) . '" size="20" />
          <label class="input__label input__label--hoshi input__label--hoshi-color-3" for="' . esc_attr( $defaults['id_username'] ) . '">
            <span class="input__label-content input__label-content--hoshi">' . esc_html( $defaults['label_username'] ) . '</span>
          </label>
			
			</span>
            <span class="input input--hoshi">
                <input type="password" name="pwd" id="password ' . esc_attr( $defaults['id_password'] ) . '" class="input__field input__field--hoshi" value="" size="20" />
          <label class="input__label input__label--hoshi input__label--hoshi-color-3" for="' . esc_attr( $defaults['id_password'] ) . '">
            <span class="input__label-content input__label-content--hoshi">' . esc_html( $defaults['label_password'] ) . '</span>
          </label>
            </span>
            ' . $login_form_middle . '
            ' . ( $defaults['remember'] ? '<p class="login-remember"><label><input name="rememberme" type="checkbox" id="' . esc_attr( $defaults['id_remember'] ) . '" value="forever"' . ( $defaults['value_remember'] ? ' checked="checked"' : '' ) . ' /> ' . esc_html( $defaults['label_remember'] ) . '</label></p>' : '' ) . '
            <p class="login-submit">
                <input type="submit" name="wp-submit" id="' . esc_attr( $defaults['id_submit'] ) . '" class="button button-primary" value="' . esc_attr( $defaults['label_log_in'] ) . '" />
                <input type="hidden" name="redirect_to" value="' . esc_url( $defaults['redirect'] ) . '" />
            </p>
            ' . $login_form_bottom . '
        </form>';
...
function wppb_front_end_login( $atts ){
	...
	if( !is_user_logged_in() ){
		...
		$login_form .= pf_login_form( apply_filters( 'wppb_login_form_args', $form_args ) );
